"""Pipedrive CRM backend for Meridian Clinic AI receptionist."""
...
class PipedriveCRMBackend(CRMBackend):
    ...
    def __init__(self) -> None:
        ...
        self.lead_title_prefix = os.getenv(
            "PIPEDRIVE_LEAD_TITLE_PREFIX", "Meridian Clinic inbound"
        )
    ...
    def _api_request(
        ...
        *,
        ...
        try:
            ...
            raise RuntimeError(
                f"Pipedrive API error {exc.code} on {path}: {detail}"
            ) from exc

        if not payload.get("success", True) and payload.get("error"):
            raise RuntimeError(
                f"Pipedrive API returned error on {path}: {payload.get('error')}"
            )
        return payload
    ...
    async def _create_person(
        self, phone: str | None, name: str | None = None
    ) -> dict[str, Any]:
        body: dict[str, Any] = {}
        if phone:
            body["phone"] = [{"value": phone, "primary": True, "label": "mobile"}]
        body["name"] = name or phone or "Inbound caller"
        payload = await self._request("POST", "/persons", body=body)
        person = payload.get("data") or {}
        return self._map_person(person)

    async def _create_pipedrive_lead(
        self, person_id: int, call_id: str
    ) -> dict[str, Any] | None:
