"""CRM backends for Meridian Clinic AI receptionist (JSON demo or Pipedrive)."""
...
DEFAULT_PHONE_COUNTRY_CODE = os.getenv("DEFAULT_PHONE_COUNTRY_CODE", "65").strip()
CUSTOMERS_PATH = Path(
    os.getenv("CUSTOMERS_PATH", str(HERE / "data" / "customers.json"))
).resolve()
LEADS_PATH = Path(os.getenv("LEADS_PATH", str(HERE / "data" / "leads.json"))).resolve()
...
def normalize_phone(value: str | None) -> str:
    ...
    if len(digits) == 8 and DEFAULT_PHONE_COUNTRY_CODE:
        # Local mobile without country code
        digits = DEFAULT_PHONE_COUNTRY_CODE + digits
    elif len(digits) == 10 and DEFAULT_PHONE_COUNTRY_CODE and not digits.startswith(
...
class JsonCRMBackend(CRMBackend):
    ...
    async def get_customer_by_phone(self, phone: str | None) -> dict[str, Any] | None:
        ...
        for customer in self._load_customers().get("customers", []):
            ...
            cust_digits = normalize_phone(customer.get("phone"))
            if cust_digits and (
                cust_digits.endswith(target[-8:]) or target.endswith(cust_digits[-8:])
            ):
                return customer
        return None
