"""Pipedrive CRM client for VibeRider Resolve — mocks to local JSON when unconfigured."""
...
MOCK_DIR = Path(__file__).resolve().parent / "data" / "cases"
...
class PipedriveClient:
    ...
    def __init__(
        ...
        if self.mock_mode:
            logger.warning(
                "PIPEDRIVE_API_TOKEN not set — Pipedrive writes will be mocked to %s",
                MOCK_DIR,
            )
    ...
    def _stage_id_for(self, case_type: str) -> int | None:
        ...
        try:
            ...
            logger.warning(
                "Ignoring invalid stage id for case_type=%s: %r", case_type, raw
            )
            return None
    ...
    def _save_mock(self, record: dict[str, Any]) -> Path:
        MOCK_DIR.mkdir(parents=True, exist_ok=True)
        path = MOCK_DIR / f"{record['deal_id']}.json"
    ...
    def create_case(
        ...
        *,
        ...
        note_body = (
            "<p><b>VibeRider Resolve report</b></p>"
            f"{case_line}"
            f"{detail_line}"
            f"<p><b>Ride</b><br/>"
            ...
            "<p><i>Created automatically by the VibeRider Resolve voice agent.</i></p>"
        )
        ...
        if self.mock_mode:
            ...
            path = self._save_mock(record)
            logger.info(
                "[MOCK Pipedrive] deal_id=%s title=%s → %s", deal_id, title, path
            )
