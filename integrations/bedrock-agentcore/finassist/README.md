...
- [AgentDuet](https://pypi.org/project/agentduet/) `==1.0.0b10` — API key + connector UUID
...
Note: Requires an IAM role with CloudFormation + AgentCore deploy permissions (Bedrock-only access is not enough).
...
For cloud deployment, make sure `AGENTDUET_API_KEY` and `AGENTDUET_CONNECTOR_UUID` are configured in `agentcore.json` env vars or AgentCore credentials, as `.env.local` is used for local development only.
